...
echo ""
echo -e "${GREEN}1. Deploy Infrastructure (Terraform)${NC}"
echo "   cd enviroments/dev"
echo "   tofu init"
echo "   tofu plan -var-file=terraform.tfvars"
echo "   tofu apply -var-file=terraform.tfvars"
echo ""
echo -e "${GREEN}2. Configure EKS Access${NC}"
echo "   aws eks update-kubeconfig --name dev-CP-EKS-CLUSTER --region ${AWS_REGION:-us-west-1}"
echo "   kubectl get nodes"
echo ""
echo -e "${GREEN}3. Deploy Microservice (Automated)${NC}"
echo "   cd microservice"
echo "   source ../.env  # Load environment variables"
echo "   ./deploy.sh \${ECR_REGISTRY} \${AWS_ACCOUNT_ID}"
echo ""
echo -e "${GREEN}4. Test the Service${NC}"
echo "   export LB_URL=\$(kubectl get ingress email-processor-ingress -n email-processor -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')"
echo "   ./test.sh http://\${LB_URL}"
echo ""
echo -e "${BLUE}════════════════════════════════════════════════════════════════${NC}"
echo ""
echo -e "${YELLOW}📚 Documentation:${NC}"
echo "  - README.md                 - Project overview"
echo "  - microservice/SETUP.md     - Detailed setup guide"
echo "  - QUICK_REFERENCE.md        - Common commands"
echo "  - DEPLOYMENT_CHECKLIST.md   - Pre-flight checklist"
echo "  - PROJECT_COMPLETE.md       - What was created"
echo ""
echo -e "${YELLOW}🆘 Need Help?${NC}"
echo "  - See QUICK_REFERENCE.md for common commands"
echo "  - Check SETUP.md for troubleshooting"
echo "  - Review logs: kubectl logs -f deployment/email-processor -n email-processor"
echo ""
